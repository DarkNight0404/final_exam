...
function ensureSummaryUI() {
    ...
    if (!modal) {
        ...
        const ul = el("ul", { className: "summary-list" });
    ...
    function renderSummaryList() {
        ...
        if (entries.length === 0) {
            const li = el("li", { className: "summary-item" });

            const row = el("div", { className: "row" });
            row.appendChild(el("div", {})).appendChild(el("b", { text: "No entries yet" }));
            row.appendChild(el("span", { className: "badge na", text: "N/A" }));

            const meta = el("div", {
                className: "meta",
                text: "Enter values on the cards and press Determine to generate a summary.",
            });
        ...
        for (const [name, r] of entries) {
            ...
            const row = el("div", { className: "row" });
            const left = el("div");
            left.appendChild(el("b", { text: name }));
            ...
            const badge = el("span", { className: `badge ${badgeClass}`, text: badgeLabel });
            ...
            if (r.enteredText) {
                meta.appendChild(document.createTextNode("Entered: "));
                meta.appendChild(el("b", { text: r.enteredText }));
            } else {
                meta.appendChild(document.createTextNode("No value entered"));
            }

            if (r.typeLabel) meta.appendChild(document.createTextNode(` · Type: ${r.typeLabel}`));
            if (r.normalText) meta.appendChild(document.createTextNode(` · Normal: ${r.normalText}`));
...
addCellCard({
    name: "Neutrophil",
    imgSrc: "./img/Picture1.png",
    shortDescription: "Left shift”: Increase in immature neutrophils (band cells), indicating active infection. Main role: First-line defense against bacteria (phagocytosis)",
    reference: {
        min: 2000,
        max: 7000,
        unit: "cells/µL",
